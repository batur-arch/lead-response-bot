 ...
 * @param {string} broker   - HVAC/Roofing company name
 * @returns {string}        - SMS-ready first message
 */
export function buildFirstMessage(lead, broker) {
  const firstName = lead.name?.split(' ')[0] ?? 'there';
  const city = lead.city ?? 'your area';
  const service = lead.service || lead.property || 'service'; // "AC repair", "roof inspection", etc.
  ...
  return (
    `Hi ${firstName}, AI assistant for ${broker} here about your ` +
...
export function buildHelpMessage(broker, brokerEmail) {
  return `${broker}: Reply STOP to opt out. Contact: ${brokerEmail}. For emergencies, call us directly. Standard msg & data rates may apply.`;
}
