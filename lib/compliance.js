...
/**
 * Build the very first message to a brand-new lead.
 * MUST include AI disclosure + broker name + STOP keyword info.
 *
 * @param {object} lead     - { name, city, property, broker }
 * @param {string} broker   - brokerage name
...
export function buildFirstMessage(lead, broker) {
  ...
  //  2. Broker name (TCPA brand ID, transactional context)
  //  3. Reference to their inquiry (transactional, not marketing)
  //  4. Single qualifier question (short)
  //  5. STOP keyword info (TCPA opt-out availability)

  return (
    `Hi ${firstName}, this is the AI assistant for ${broker} ` +
    `about your inquiry in ${city}. ` +
    `Quick question — are you pre-approved for a mortgage, ` +
    `or still exploring? Reply STOP to unsubscribe.`
  );
}
...
export function buildOptOutConfirmation(broker) {
  return `You've been unsubscribed from ${broker} messages. Reply START to re-subscribe. No further messages will be sent.`;
}
...
export function buildHelpMessage(broker, brokerEmail) {
  return `${broker}: Reply STOP to opt out. Contact: ${brokerEmail}. Standard msg & data rates may apply.`;
}
